      ...
      <ul className="mt-12 grid w-full list-none gap-4 p-0 md:grid-cols-[repeat(auto-fill,minmax(250px,1fr))] lg:grid-cols-[repeat(auto-fill,minmax(300px,1fr))]">
        {projectsToShow.map((project) => (
          <li key={project.title} className="group relative">
            <div className="flex h-full flex-col justify-between overflow-auto rounded-(--border-radius) bg-(--light-navy) px-7 py-8 shadow-[0_10px_30px_-15px_var(--navy-shadow)] transition duration-200 group-hover:-translate-y-1.5 group-hover:shadow-[0_20px_30px_-15px_var(--navy-shadow)]">
              <header>
